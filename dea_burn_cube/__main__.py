...
def get_geomed_ds(region_id, split_count, period, hnrs_config, geomed_bands):
    ...
    target_dataset = [e for e in metadata_files if e.metadata_doc["properties"]["odc:region_code"] == region_id][0]
    ...
    geomed = hnrs_dc.load(datasets=[metadata], output_crs="EPSG:3577", resolution=(-30, 30), measurments=geomed_bands, dask_chunks={})
...
def generate_subregion_result(dc, geomed, ard_bands, period, mappingperiod, polgyon_geometry, x_i, y_i, split_count, x_code, y_code):
    
    gpgon = datacube.utils.geometry.Geometry(polgyon_geometry, crs="epsg:3577")
    
    ard = dea_tools.datahandling.load_ard(dc, products=['ga_ls8c_ard_3'], measurements=ard_bands,
                                     geopolygon=gpgon,
...
def burn_cube_run(
    ...
    ard_bands = [f'nbart_{band}' for band in ('red', 'green', 'blue', 'nir', 'swir_1', 'swir_2')]

...
def post_filtering(sev,date_filtering=True,hotspots_filtering=True):
    ...
    if date_filtering==True:
        ...
        if len(sortcounts>0):
            ...
            fireevents = sortcounts[0:len(sortcounts)]
        if len(values)>1&~np.isnan(np.nansum(values)):
            for fire in fireevents:
                print('Change detected at: ',values[counts==fire].astype('datetime64[ns]')[0])
                firedates=values[counts==fire]
                for firedate in firedates:
                    ...
                    print(start,end,sev.StartDate.shape,sev.Duration.shape)
    ...
    if hotspots_filtering==True:
        ...
        if ('Corroborate' in sev.keys())*(sev.Corroborate.data.sum()>0):
            ...
            overlaplabels,labelcounts = np.unique(tmp, return_counts=True)
            labels = overlaplabels[overlaplabels>1]
